    ...
    executeCommand(command) {
        ...
        if (giveMatch) {
            ...
            if (amount > 0) {
                ...
                this.consoleOutput.innerHTML = `<i class="fas fa-check-circle"></i> Добавлено ${amount} кристаллов`;
                this.consoleOutput.style.color = '#00ff00';
            } else {
                this.consoleOutput.innerHTML = `<i class="fas fa-exclamation-circle"></i> Количество должно быть больше 0`;
                this.consoleOutput.style.color = '#ff0000';
            }
        } else if (idMatch) {
            const newId = parseInt(idMatch[1]);
            if (newId > 0) {
                const formattedId = `P${newId.toString().padStart(4, '0')}`;
                localStorage.setItem('snakePlayerId', formattedId);
                this.updateIdDisplay(formattedId);
                this.consoleOutput.innerHTML = `<i class="fas fa-check-circle"></i> ID изменен на ${formattedId}`;
                this.consoleOutput.style.color = '#00ff00';
            } else {
                this.consoleOutput.innerHTML = `<i class="fas fa-exclamation-circle"></i> ID должен быть больше 0`;
                this.consoleOutput.style.color = '#ff0000';
            }
        } else {
            this.consoleOutput.innerHTML = `<i class="fas fa-exclamation-circle"></i> Неверная команда. Используйте: /give &lt;количество&gt; или /idnum &lt;номер&gt;`;
            this.consoleOutput.style.color = '#ff0000';
        }
        
        // Add animation to output
        this.consoleOutput.style.animation = 'none';
        this.consoleOutput.offsetHeight; // Trigger reflow
        this.consoleOutput.style.animation = 'consoleFadeIn 0.3s ease-out';
    }
